...
use app\manager\model\ArticleCategory as CategoryModel;
...
class Articlecategory
{
    ...
    {
        $where = [
            ['category_name','like','%'.$postData['category_name'].'%']
        ];
    ...
    {
        $data = $this->model->ccOne($postData['category_id']);
    ...
    {
        $data = $this->model->ccDel($postData['category_id']);
        return R::json(0, "", lang('operation_success'));
    }
}